...
function buscar() {
    ...
    $.ajax({
        ...
        success: function(data, textStatus, jqXHR) {
            ...
            var n = 0;
            $.each(data.cliente, function(i, item) {

                html += '<option value="" >' + item[n].nombre + ' ' + item[n].Apellido + '</option>';

                n++;
            });
            $.each(data.vehiculo, function(i, item) {

                opt += '<option value="" >' + item[n].placa + ' </option>';
                n++;
            });
            $("#nombre").html(html);
            $("#placa").html(opt);
