...
def nuevoalquiler(request):
    ...
    if usuario.groups.filter(name = 'Administrador').exists():
        ...
        alquiler.placa_vehiculo = request.get('placa')
        alquiler.fechaInicio = request.get('fechaini')
        alquiler.fechaFin = request.get('fechafin')
...
def busqueda(request):
    cliente = Cliente.objects.all().values("nombre", "Apellido");
    vehiculo = Vehiculo.objects.all().values("placa","marca","color");
    lista = {
